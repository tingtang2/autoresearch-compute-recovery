...
class Interpreter:
    ...
    def run(self, code: str, id, reset_session=True):
        ...
        # 60s timeout: under concurrent heavy training, process fork + setup can be slow
        try:
            state = self.event_outq[process_id].get(timeout=60)
        except queue.Empty:
        ...
        _eof_timeout = 60  # seconds to wait for EOF after child finished
        while not self.result_outq[process_id].empty() or not output or output[-1] != "<|EOF|>":
            try:
                res = self.result_outq[process_id].get(timeout=_eof_timeout)
            except _queue_mod.Empty:
                logger.warning(f"Timed out waiting for EOF from child (process_id={process_id}), breaking out")
                output.append("<|EOF|>")
                break
            output.append(res)
