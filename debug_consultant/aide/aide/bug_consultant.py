...
class BugConsultant:
    ...
    def __init__(
        ...
        logger.info("🗂️  Debug consultant initialized (model: %s)", model)
    ...
    def format_context_for_actor(self, retrieval_result: dict) -> str:
        ...
        Plain format (86% success vs 31% scary format in ablation tests)
        - Simple "BANNED:" prefix for failed approaches
        - "PROVEN FIX:" only for bugs that were successfully fixed
        """
        # Early stopping guidance (always include)
        early_stopping = (
            "PROVEN FIX:\n"
        ...
        # Collect all failed strategies AND original error signatures
        banned_items = []
        ...
        # Always include banned early stopping patterns (debug context needs this too)
        ...
        # Only show PROVEN FIX if bug was successfully fixed
        if proven_fixes:
            lines.append("")
    ...
    def get_prevention_guidance(self, mode: str = "executive", journal: Optional["Journal"] = None) -> str:
        ...
        """
        # Always include early stopping guidance even when no bugs yet
    ...
    def _distill_guidance(self, raw_world_model: str) -> str:
        ...
        try:
            ...
            # Convert structured result to pattern-based format
            patterns = result.get("patterns", [])
            if not patterns:
                return self._distill_guidance_fallback(proven_failures, proven_successes)

            # New format - "BANNED:" with pattern description and error type
            ...
            # Always include early stopping guidance (proven fix)
    ...
    def _distill_guidance_fallback(self, proven_failures: list, proven_successes: list) -> str:
        """Fallback: Simple deterministic format when LLM fails. Pattern-based format."""
        lines = ["BANNED:"]
