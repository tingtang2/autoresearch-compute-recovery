# AIDE (vendored, with the debug consultant)

This directory is a vendored copy of **AIDE ML** by [WecoAI](https://github.com/WecoAI/aideml)
(MIT license — see `LICENSE`), modified to add the debug consultant.

| File | Change |
| --- | --- |
| `aide/bug_consultant.py` | added — the debug consultant |
| `aide/agent.py` | records bugs after each node, injects prevention guidance into prompts |
| `aide/utils/config.yaml` | `agent.search.use_bug_consultant` and related keys (line 66) |

See `../README.md` for how to install and run this under MLE-bench (do **not**
`pip install aideml` — install this directory with `pip install -e .`). The `Dockerfile` and
`Makefile` here are upstream standalone artifacts and are not used by the MLE-bench setup.
