                ...
                if self.is_root(parent_node):
                    if parent_node.reached_child_limit(scfg=self.scfg):
                        ...
                        if result_node:
                            ...
                            # Livelock guard: draft limit reached + 0 successful nodes anywhere is unrecoverable.
                            if self.best_node is None:
                                logger.error(
                                    "Livelock guard: draft limit reached, aggregation impossible "
                                    "(0 successful branches), and no node has produced a valid metric. "
                                    "Terminating search."
                                )
