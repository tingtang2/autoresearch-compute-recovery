...
CHANGES vs agent_search.py:
- Added `should_restart`, `restart_attempts`, `max_restart_attempts` state on AgentSearch.
- Livelock branch in `_run_single_step` now requests a restart (with BANNED-rule injection
  via the preserved BugConsultant) instead of permanently terminating, until the restart
  budget is exhausted.
- Added `prepare_for_restart()` helper that quiesces dead drafts, resets virtual_root
  counters, and preserves the BugConsultant (so `_global_conditional_rules` survive).
"""
...
class AgentSearch:
    def __init__(
        ...
        # ── NEW: livelock-restart state ─────────────────────────────────────────────
        # When the livelock guard fires, we set should_restart=True so the scheduler
        # can drain in-flight work and re-run Phase-1 draft generation with the
        # BugConsultant's accumulated BANNED rules injected via get_prevention_guidance().
        # We preserve the BugConsultant across restarts so _global_conditional_rules
        # is carried forward.
        self.should_restart: bool = False
        self.restart_attempts: int = 0
        self.max_restart_attempts: int = getattr(self.scfg, "max_restart_attempts", 2)
        ...
        logger.info(f"[MCTS] Restart budget: max_restart_attempts={self.max_restart_attempts}")
    ...
    def prepare_for_restart(self):
        """Quiesce dead drafts and reset virtual_root counters for a fresh Phase-1 pass.

        Preserves: BugConsultant, _global_conditional_rules, parent_conditional_rules,
        on-disk bug records, learned global_memory, journal entries (with stage tagged).

        Idempotent under concurrent invocation via _restart_lock.
        """
        with self._restart_lock:
            if not self.should_restart:
                return  # Another thread already handled the restart request.

            logger.warning(
                f"[restart] Preparing search for restart attempt "
                f"{self.restart_attempts + 1}/{self.max_restart_attempts}."
            )

            for child in list(self.virtual_root.children):
                # Lock and terminalize so UCT/Top-K skip these dead drafts forever.
                child.lock = True
                child.is_terminal = True
                if hasattr(child, "stage"):
                    child.stage = "dead_draft"

            # Reset the counter that gates new Phase-1 draft generation. The dead
            # drafts remain in branch_all_nodes (informative); branch_successful_nodes
            # is already empty for them, so aggregation correctly skips them.
            try:
                self.virtual_root.expected_child_count = 0
            except AttributeError:
                pass

            # Untag the virtual_root from being terminal so a new Phase-1 pass can
            # generate fresh children from it.
            self.virtual_root.is_terminal = False

            self.restart_attempts += 1
            self.should_restart = False
            logger.info(
                f"[restart] Quiesced {sum(1 for _ in self.virtual_root.children)} prior drafts; "
                f"BugConsultant retained with "
                f"{len(getattr(self.bug_consultant, '_global_conditional_rules', []) or []) if self.bug_consultant else 0} "
                f"global BANNED rules."
            )
                ...
                if self.is_root(parent_node):
                    if parent_node.reached_child_limit(scfg=self.scfg):
                        ...
                        if result_node:
                            ...
                            # ── MODIFIED: Livelock guard now requests a restart instead of permanently terminating ──
                            if self.best_node is None:
                                with self._restart_lock:
                                    if self.restart_attempts < self.max_restart_attempts:
                                        logger.warning(
                                            f"Livelock guard: 0 successful branches and no valid metric. "
                                            f"Requesting restart with BANNED rules "
                                            f"({self.restart_attempts + 1}/{self.max_restart_attempts})."
                                        )
                                        ...
                                        logger.error(
                                            f"Livelock guard: restart budget exhausted "
                                            f"({self.max_restart_attempts}). Terminating search."
                                        )
                                        self.virtual_root.is_terminal = True
                                        return _root, None
