...
"""
    ...
    def get_node_reward(self, node: MCTSNode):
        ...
        # After baseline phase (+ >30% steps): shape reward by hpo_score; penalize repeated HPO signatures
        ...
        # Prefer a single model early; lightly penalize early ensembling
        model_keywords = [
    ...
    def step(self, node: MCTSNode, exec_callback: ExecCallbackType) -> bool:   
        ...
        if result_node and result_node.metric.value is not None:
            if self.in_baseline_phase:
                ...
                # Only promote post-baseline if the node beats the captured baseline
                promote = (
